...
TOOLS = [
    ...
    Tool(
        ...
        description=(
            "【清空政策知识库工具】\n"
            "功能：清空 policy_search 的结构化政策知识库（knowledge_base/ 目录下所有 {doc_id}.json 政策文件及 index.json 索引）。\n"
            "这些数据是通过 load_policy_document 加载并经 LLM 抽取的结构化政策条件，清空后需要重新加载文档才能恢复。\n"
            "操作不可恢复，执行前必须向用户二次确认意图。\n"
            "\n"
            "适用范围：仅清理本 policy_search server 的知识库。\n"
            "⚠️ 不要用于清空 RAG 的向量知识库（chroma_data/ 中的向量与 BM25 索引）——\n"
            "   那是另一个 MCP server (rag)，请改调用 rag 的同名工具。\n"
            "\n"
            "触发条件：用户明确提到以下关键词，且上下文是针对政策库：\n"
            "- 清空政策知识库、删除所有政策、重置政策库\n"
            "- 清除政策数据、清空已加载的政策\n"
            "\n"
            "区分提示：如果用户说的是\"清空 RAG / 清空向量库 / 清空检索文档\"，那是 rag server 的职责，不要调用本工具。"
        ),
        inputSchema={
            "type": "object",
            "properties": {},
