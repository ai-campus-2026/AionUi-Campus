...
export function useWorkbench(): WorkbenchApi {
  ...
  const sendToAi = useCallback(
    async (
      ...
      loadLatestConversationMessages(conv.id, { limit: 10, contentMode: 'compact' })
        .then((p) => {
          const prev = [...(p.items ?? [])].toReversed().find((m) => m.type === 'text' && m.position === 'left');
  ...
  const deleteTask = useCallback((taskId: string) => {
    setState((s) => ({
      ...
      conversations: s.conversations.filter(
        (conv) => !s.tasks.find((t) => t.id === taskId && t.conversationId === conv.id)
      ),
    }));
  }, []);
  ...
  const deleteTasks = useCallback((taskIds: string[]) => {
    ...
    setState((s) => ({
      ...
      conversations: s.conversations.filter(
        (conv) => !s.tasks.find((t) => ids.has(t.id) && t.conversationId === conv.id)
      ),
    }));
  }, []);
