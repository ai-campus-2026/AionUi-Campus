    ...
    <div className="ap-history">
      <header className="ap-history__header">
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          ...
          <button type="button" className="ap-btn ap-btn--primary" onClick={onReupload}>＋ 重新上传</button>
        </div>
      </header>
      ...
      <div className="ap-history__scroll">
        ...
        {history.length === 0 ? (
          ...
          <div className="ap-history__list">
            {history.map((p) => (
              <div key={p.id} className={`ap-history__item ${p.id === currentPlanId ? 'ap-history__item--current' : ''}`}>
                ...
                <div className="ap-history__item-right">
                  ...
                  <button type="button" className="ap-btn ap-btn--ghost" onClick={() => onSwitch(p)}>查看</button>
                </div>
              </div>
            ))}
          </div>
